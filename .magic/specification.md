...
4. **Automation**: If system files are missing, offer to run the **Init Workflow** (`.magic/init.md`) immediately.
...
4. **Initialization**: If `.design/INDEX.md` or `.design/RULES.md` are missing, **STOP** and run the Init Workflow (`.magic/init.md`).
5. **Content Creation**:
...
8. **Report**: Present findings before making any changes:
    ...
    - RON format used independently in 3 specs
...
> `INDEX.md` and `RULES.md` are generated by the **Init Workflow** (`.magic/init.md`).
> Run `.magic/scripts/init.sh` (macOS/Linux) or `.magic/scripts/init.ps1` (Windows) to create them.
